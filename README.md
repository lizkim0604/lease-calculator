...
처음엔 "계산 공식이 기준서와 구조적으로 맞으면 된다"고 생각했지만, 실제로는
그렇지 않았습니다. 검증 과정에서 리스채권 금액 불일치, 분개 차변≠대변,
선급/후급 시점 처리 누락, 잔존가치 보증 주체 판단 오류 등 실제 계산 오류를
여러 차례 발견하고 수정했습니다. "AI가 기준서를 안다"는 것과 "그 지식을 정확한
계산 로직으로 옮겼다"는 것은 완전히 다른 문제라는 걸 이 프로젝트를 통해
확인했고, 그래서 조문을 원문 단위로 대조하고 매 계산마다 기계적으로 검증
가능한 기준(분개의 차변=대변)을 코드에 넣는 방향으로 계속 다듬고 있습니다.

## 면책 사항

이 도구는 계산 편의를 위한 보조 자료이며, 회계 감사 근거 자료로 그대로 사용할
수 없습니다. AI 추출 결과는 반드시 계약 원문과 대조해 확인해야 합니다.
